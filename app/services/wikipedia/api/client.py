...
class WikipediaAPIClient:
    ...
    async def fetch_pages_batch(
        ...
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    ...
                    delay = min(
                        base_delay * (2 ** (attempt - 1)) * (0.5 + 0.5 * random.random()),
                        60.0,  # Max 60 seconds
                    )
                ...
                if 400 <= e.status < 500 and e.status != 429:
                    raise WikipediaAPIError(
                        ...
                        response={"status": e.status, "headers": dict(e.headers)},
        ...
        if isinstance(last_exception, asyncio.TimeoutError):
            raise WikipediaAPITimeoutError(timeout=self._timeout.total, url=base_url) from last_exception
        elif last_exception:
            raise WikipediaAPIError(
