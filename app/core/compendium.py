...
def extract_topic_from_linker_data(linker_data: dict) -> str:
    """Extract main topic from linker output data."""
    # Use original text if available
    original_text = linker_data.get("original_text", "")
    if original_text:
        return extract_topic_from_text(original_text)

    # Fallback: use most prominent entity
    entities = linker_data.get("entities", [])
    if entities:
        return entities[0].get("entity", "Unbekanntes Thema")

    return "Unbekanntes Thema"


def extract_references_from_linker_data(linker_data: dict) -> list[str]:
...
def create_entity_context(linker_data: dict) -> str:
...
def generate_compendium_with_openai(topic: str, context: str, references: list[str], config) -> str:
...
def generate_compendium_from_text(text: str, config) -> tuple[str, str, dict]:
...
def generate_compendium(linker_data: dict, config) -> tuple[str, str, dict]:
...
def generate_compendium_legacy(entities: list[dict]) -> str:
