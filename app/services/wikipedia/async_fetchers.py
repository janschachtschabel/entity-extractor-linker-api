...
__all__ = ["get_redirect_targets", "html_extract", "open_search"]


async def get_redirect_targets(title: str, *, lang: str, session) -> list[str]:
...
async def html_extract(title: str, *, lang: str, session) -> str | None:
    """Fetch the article HTML and return first paragraph text.

    Only used as last-resort fallback; avoids full MediaWiki API dependency.
    """
    url = f"https://{lang}.wikipedia.org/wiki/{title}"
    logger.debug("HTML fetch %s", url)
    try:
        async with session.get(url, timeout=10) as resp:
            if resp.status != 200:
                return None
...
async def open_search(term: str, *, lang: str, session) -> list[str]:
